...
public class ExcelUtils {
		...
		for (int j = 0; j < length; j++) {
			...
			if (cell.getContents() != null && !"".equals(cell.getContents())) {
				try {
					...
					} else if (returnType.toString().indexOf("java.lang.Integer") != -1) {
						if (value != null) {
							ReflectHelper.setValue(instance, mappingFields[j],new Integer(value));
						}
					} else if (returnType.toString().indexOf("java.util.Date") != -1) {
						if (value != null) {
							value = value.trim();
							String dateFormat = "";
							if(value.length()==10) dateFormat = TimeUtil.FORMAT_DATE_ONLY;
							else if(value.length()==19) dateFormat = TimeUtil.FORMAT_NORMAL;
							else throw new Exception(String.format("非法日期格式（合法为%s、%s）:%s",TimeUtil.FORMAT_DATE_ONLY,TimeUtil.FORMAT_NORMAL,value));
	 ...
	 * @throws Exception
	 */
	public static List readSheet(Class clazz, String[] mappingFields, int sheetIndex, int startLine, InputStream is) 
		throws Exception {
		List list = new ArrayList();
